...
\]
%In summary, by modeling the time difference of riders to the race leader with a compound Poisson-Gamma model, we end up with a Tweedie distribution.
...
\]
The first term in Eqn. \refeq{eq:loglike} is the log-likelihood of \iid Tweedie observations. The second term can be thought of as a penalty: it takes the value of 0 when each stage has an observation that is zero, and the value of $-\infty$ otherwise. In the case a stage does not have an observation that is zero, the MLE is not defined. Else, the MLE is that of the Tweedie distribution. \\
...
\]
Under the log response function, the covariates have a multiplicative effect on the mean of the response, which makes the model easier to interpret. Another alternative is to use the (scaled) canonical response function which, following Eqn. \refeq{eq:canonical}, for this case is given by
\[
